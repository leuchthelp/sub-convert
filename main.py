...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Extracting and converting tracks if older(+)/younger(-) than amount of offset from current date specified. "
            + 'Default: "", means nothing will be skipped. Given as str i.e. \"H+8\" = \"process older than 8 hours\". '
            + "Will rerun the whole MKV file as soon as it finds one SRT track +/- the threshold as SRT file-names "
            + "generated by this tool cannot be inferred back."
        ),
    )
    parser.add_argument(
        "-cw",
